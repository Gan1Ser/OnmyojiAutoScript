...
from tasks.GameUi.page import page_main, page_netherworld, page_kirin, page_shikigami_records
...
class ScriptTask(GameUi, GeneralBattle, GeneralInvite, SwitchSoul, HuntAssets):
    ...
    def check_datetime(self) -> bool:
        ...
        now = datetime.now()
        # 如果时间在00:00-6:00 之间则设定时间为当天的自定义时间，返回False
        ...
        elif now.time() > time(23, 0) and self.kirin_day:
            self.plan_tomorrow_hunt()
            raise TaskEnd('Hunt')
